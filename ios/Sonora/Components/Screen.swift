...
private struct SonoraScrollOffsetKey: PreferenceKey {
...
struct SonoraScreen<Trailing: View, Content: View>: View {
    let title: String
    var eyebrow: String?
        ...
        ScrollView {
            ...
            .background(alignment: .top) {
                GeometryReader { proxy in
                    Color.clear.preference(key: SonoraScrollOffsetKey.self, value: proxy.frame(in: .named(space)).minY)
                }
                .frame(height: 0)
            }
        }
...
extension SonoraScreen where Trailing == EmptyView {
