...
import { quote, refundAmount, refundPercent } from "./pricing.ts";
...
Deno.test("quote without add-ons", () => {
  ...
  assertEquals(p.service_fee, 600);
  assertEquals(p.total, 8100);
  assertEquals(p.due_now, 8100);
  assertEquals(p.studio_commission, 375);
  assertEquals(p.studio_payout, 7125);
});
...
Deno.test("quote with add-ons and deposit", () => {
  ...
  assertEquals(p.service_fee, 2000);
  assertEquals(p.deposit_amount, 7500);
  assertEquals(p.due_now, 9500);
...
Deno.test("refund policies", () => {
  ...
  assertEquals(refundAmount(p, p.total, "moderate", in30h, "artist", now), 3750); // 50% of 7500, fee kept
  assertEquals(refundAmount(p, p.total, "strict", in30h, "studio_owner", now), 8100); // studio cancels: full
});
